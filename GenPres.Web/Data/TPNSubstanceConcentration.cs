...
namespace GenPres
{
    public class TPNSubstanceConcentration
    {
        ...
        {
            _prescription = p;
            string concentrations = System.IO.File.ReadAllText(Settings.SettingsManager.Instance.GetSettingsPath() + "tpn_concentrations.csv");
            ...
            for (int l = 1; l < concentrationLines.Length; l++)
            {
                ...
                {
                    decimal val = Decimal.Parse(substanceQuantities[s], System.Globalization.CultureInfo.CreateSpecificCulture("en-US"));
