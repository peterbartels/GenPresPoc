...
namespace GenPres.Operations.Calculation
{
    public class AbstractCombination: ICalculationCombination
    {
         ...
         */
        internal int? GetFieldIndexByState(StateManager.ValueState state)
        {
        ...
        {
            int value = -1;
            PropertyInfo<UnitValue> prop = (from i in _properties where _propertyManager.GetState(i) == StateManager.ValueState.NotSet select i).FirstOrDefault<PropertyInfo<UnitValue>>();
        ...
        {
            if (_propertyManager.GetState(property) == StateManager.ValueState.User) 
                return true;
        ...
        internal PropertyInfo<UnitValue> GetPropertyToRectify()
        {
            return (
                ...
                where
                    (_propertyManager.GetState(i) == StateManager.ValueState.User || _propertyManager.GetState(i) == StateManager.ValueState.Calculated)
        ...
        internal void CorrectPropertyValues()
        {
            if (GetNotSetCount() > 0)
            {
                ...
                {
                    if (_propertyManager.GetState(_properties[i]) == StateManager.ValueState.User)
        ...
        public void CalculateSibblings(int index)
        {
            ...
            {
                if (pc.Contains(GetAt(index)) && pc != this)
                {
                    pc.SetLastPropertyCalculated(GetAt(index));
                    pc.Calculate();
        ...
        {
            return _propertyManager.GetState(property) == StateManager.ValueState.NotSet;
        }
        ...
        */
        ...
        {
            return _propertyManager.GetState(property) == StateManager.ValueState.User;
        }
        #endregion
